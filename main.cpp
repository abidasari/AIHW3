...
string get_args(string x) //returns the arguments list
{
	string delimiter1 = "(";
	string delimiter2 = ")";
...
string get_op(string x) // returns the name of the operand
{
	string delimiter = "(";
	int pos = x.find(delimiter);
...
string unify(string x, string y, string theta) // rhs of the matched clause in the sub_kb, query, theta
{
...
string get_rhs(string x) // returns the rhs side of the =>. if no => returns the input string
{
	string delimiter = "=>";
	int pos = x.find(delimiter);
	if (pos == -1)
...
string get_lhs(string x) // same func as the rhs
{
	string delimiter = "=>";
	int pos = x.find(delimiter);
	if (pos == -1)
...
string get_first_lhs(string x) // returns the first clause of the ^ list
{
	string delimiter = "&";
	int pos = x.find(delimiter);
	if (pos == -1)
...
string get_rest_lhs(string x) // returns the remaining ^'s
{
	string delimiter = "&";
	string empty;
	int pos = x.find(delimiter);
...
string sub(string sub, string x) //theta, first in lhs
{
...
int count_args(string x)// returns the number of arguements in the ',' separated string
{
	string delimiter = ",";
	int count = 0;
	...
	while (x.find(delimiter) != -1)
	{
		count++;
		int pos = x.find(delimiter);
		x.replace(pos, 1, ";"); // just to prevent the same comma from being counted again.
	}
	return count + 1;
}
vector<string>vectorify(string args) // converts and returns the , separated args list into a vector.
{
	string delimiter = ",";
	int pos;
	vector<string>res;
...
list<string>fetch_rules(list<string> kb, string goal)
{
	...
	{
		string temp = get_rhs(*i); //gets everything to the right of the '=>', if no '=>' returns the string itself 
		if (get_op(goal) != get_op(temp)) //get_op returns the name of the operand till '('
			continue;
		cout << *i << endl;
		cout << "op stage passed" << endl;
		string goal_args = get_args(goal); //get_args returns the , separated arguements list
		string temp_args = get_args(temp); //  same as above
		int goal_args_count = count_args(goal_args); // returns the number of arguements.
		...
		int chk = 0;
		vector<string>goal_params=vectorify(goal_args); // converts the ',' separated args list into a vector list
		vector<string>temp_params=vectorify(temp_args); // same as above
...
list<string>bc_or(list<string> kb, string query, string theta)//
{
	...
	list<string>sub_kb;
	sub_kb = fetch_rules(kb, query); // the fetch function returns the the clauses which have to be resolved ie whose rhs signature matches the signature of the query.
	list<string>::iterator i;
	list<string>res;
	for (i = sub_kb.begin(); i != sub_kb.end(); i++)
	{
		//implement standardizing here
		string lhs = get_lhs(*i); // returns the rhs of =>
		string rhs = get_rhs(*i); // returns the lhs of =>
		list<string>and_result = bc_and(kb, lhs, unify(rhs, query, theta)); 
		res.merge(and_result);
	}
	return res;
}
list<string>bc_and(list<string>kb, string query, string theta)// kb is kb, query is lhs from the res sentence, theta is returned from the unify function
{
	...
	else if (query.length() == 0)// "query" here is rhs of the => in the kb 
	{
		list<string>self;
...
{
	string theta;
	return bc_or(kb, query, theta); // not sure what theta is for
}
int main()
{
	fstream f;
